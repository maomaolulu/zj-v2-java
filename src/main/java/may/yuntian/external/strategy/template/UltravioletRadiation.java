 ...
 */
public class UltravioletRadiation implements Strategy {
    ...
    public void strategyInterfaceEv(ProResultItemVO proResultItemVO, BatchSampleDto batchSampleDto, ResultEntity resultEntity) {
        ...
        if (!NumConstants.NUMBER_FIVE.equals(sType)) {
            return;
        }
        // 5:紫外辐射,微波辐射  计量单位编码:unit
        proResultItemVO.setUnit(NumConstants.ONE_THOUSAND_EIGHT);
        // 结果
        proResultItemVO.setResult(batchSampleDto.getResult());
        // 结果项编码:code
        proResultItemVO.setCode(NumConstants.ONE_HUNDRED_AND_NINE);
        // 接触限值:limit
